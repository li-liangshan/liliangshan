...
# 接着处理下一行，这样不断重复，直到文件末尾。文件内容并没有 改变，除非你使用重定向存储输出。Sed主要用来自动编辑一个或多个文件；
# 简化对文件的反复操作；编写转换程序等。
...
echo 'sksksksksksksksksksk' | sed 's/sk/SK/2'

# 以上命令中字符 / 在sed中作为定界符使用，也可以使用任意的定界符
sed 's:test:Test:g' <<< 'test_test_hello'
sed 's|test|Test|g' <<< 'test_test_hello'
...
# -e选项允许在同一行里执行多条命令
# sed表达式的第一条命令删除1至5行，第二条命令用check替换test。命令的执行顺序对结果有影响。
...
# 在test.conf 文件第2行之后插入this is a test line
...
# 脚本文件script_file
# sed脚本是一个sed的命令清单，启动Sed时以-f选项引导脚本文件名。Sed对于脚本中输入的命令非常挑剔，在命令的末尾不能有任何空白或文本，
